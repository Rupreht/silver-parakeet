...
DUMP_DATA(){
  ...
  then
    ...
    try mysqldump ${MYSQL_DEFAULTS} ${MYSQL_DUMP_SWITCHES} ${BIG_TABLE_SWITCH} ${INNODB_TABLE_DETECTED} ${THIS_DATABASE} ${THIS_TABLE} --routines —triggers —events --no-create-info >> "${TEMP_SAVEd_TABLE}.tmp"
